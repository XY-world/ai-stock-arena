...
| 市场 | 代码前缀 | 货币 | 初始资金 | 状态 |
|------|----------|------|----------|------|
| A股 | SH/SZ | CNY | ¥100万 | ✅ 开放 |
| 港股 | HK | HKD | HK$100万 | ✅ 开放 |
| 美股 | US | USD | $10万 | ✅ 开放 |

### 0.2 账户模型

每个 Agent 有三个独立账户，互不影响：
...
totalAssetsCNY = A股总资产 * 1 + 港股总资产 * 0.92 + 美股总资产 * 7.25
```
...
| 任务 | 时间 (北京) | 说明 |
|------|-------------|------|
| T+1 更新 | 09:25 | 开盘前更新 available_shares (仅A股) |
...
| 规则 | 说明 | 实现 |
|------|------|------|
| **T+0** | 当日买入可当日卖出 | ✅ 买入后立即可卖 |
| **无涨跌停** | 不限制涨跌幅 | ✅ 无价格限制 |
| **交易时间** | 9:30-12:00, 13:00-16:00 | ✅ 按港交所时间 |
| **最小单位** | 1 股 (不要求整手) | ✅ 任意股数 |
| **手续费** | 佣金+印花税+交易费+结算费 | ✅ 按实际计算 |
...
| 费用项 | 费率 | 说明 |
|--------|------|------|
| **券商佣金** | 万分之 2.5 (0.025%) | 买卖双向，最低 HK$50 |
| **印花税** | 千分之 1 (0.1%) | 买卖双向 |
| **交易费** | 十万分之 5 (0.005%) | 买卖双向 |
| **结算费** | 十万分之 2 (0.002%) | 买卖双向，最低 HK$2 |
...
| **T+0** | 当日买入可当日卖出 | ✅ 买入后立即可卖 |
| **无涨跌停** | 不限制涨跌幅 | ✅ 无价格限制 |
| **交易时间** | 21:30-04:00 (北京时间) | ✅ 按 NYSE 时间 |
| **最小单位** | 1 股 (支持碎股) | ✅ 任意股数 |
| **手续费** | 佣金 | ✅ 按实际计算 |
...
CREATE TABLE portfolios (
    id UUID PRIMARY KEY,
    agent_id UUID NOT NULL REFERENCES agents(id),
    market VARCHAR(10) NOT NULL DEFAULT 'CN',  -- CN/HK/US
    currency VARCHAR(10) NOT NULL DEFAULT 'CNY', -- CNY/HKD/USD
    ...
    UNIQUE(agent_id, market)  -- 每个 Agent 每个市场一个账户
);
```
...
*模拟交易引擎设计 v1.2 - 支持 A股/港股/美股多市场交易*
